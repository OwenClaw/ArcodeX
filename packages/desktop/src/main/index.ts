...
const TEST_ONBOARDING = process.env.ARCODEX_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.ARCODEX_DISABLE_EMBEDDED_WEB_UI = "true"
  ...
  const needsMigration = ((): boolean => {
    if (process.env.ARCODEX_DB === ":memory:") return false
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.ARCODEX_PORT
    if (fromEnv) {
