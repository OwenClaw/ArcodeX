...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and cancels paid models without credentials", () =>
    withEnv({ ARCODEX_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        const updated = yield* plugin.trigger("provider.update", {}, { provider: provider("arcodex"), cancel: false })
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ ARCODEX_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        yield* plugin.trigger("provider.update", {}, { provider: provider("arcodex"), cancel: false })
  ...
  it.effect("uses ARCODEX_API_KEY as credentials", () =>
    withEnv({ ARCODEX_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        const updated = yield* plugin.trigger("provider.update", {}, { provider: provider("arcodex"), cancel: false })
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ ARCODEX_API_KEY: undefined, CUSTOM_ARCODEX_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        const updated = yield* plugin.trigger(
          "provider.update",
          {},
          { provider: provider("arcodex", { env: ["CUSTOM_ARCODEX_API_KEY"] }), cancel: false },
        )
        const paid = yield* plugin.trigger(
          "model.update",
          {},
    ...
    withEnv({ ARCODEX_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const updated = yield* plugin.trigger(
          "provider.update",
          {},
          { provider: provider("arcodex", { enabled: { via: "auth", service: "opencode" } }), cancel: false },
        )
        const paid = yield* plugin.trigger(
          "model.update",
          {},
