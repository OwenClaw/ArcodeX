...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-arcodex-session") ?? ""
    const requestId = input.request.headers.get("x-arcodex-request") ?? ""
    const projectId = input.request.headers.get("x-arcodex-project") ?? ""
    const ocClient = input.request.headers.get("x-arcodex-client") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-arcodex-request")
          headers.delete("x-arcodex-session")
          headers.delete("x-arcodex-project")
          headers.delete("x-arcodex-client")
          return headers
        })(),
        body: reqBody,
      })
