...
const original = {
  ARCODEX_SERVER_PASSWORD: Flag.ARCODEX_SERVER_PASSWORD,
  ARCODEX_SERVER_USERNAME: Flag.ARCODEX_SERVER_USERNAME,
  envPassword: process.env.ARCODEX_SERVER_PASSWORD,
  envUsername: process.env.ARCODEX_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.ARCODEX_SERVER_PASSWORD = original.ARCODEX_SERVER_PASSWORD
  Flag.ARCODEX_SERVER_USERNAME = original.ARCODEX_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.ARCODEX_SERVER_PASSWORD
  else process.env.ARCODEX_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.ARCODEX_SERVER_USERNAME
  else process.env.ARCODEX_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.ARCODEX_SERVER_PASSWORD = auth.password
  Flag.ARCODEX_SERVER_USERNAME = auth.username
  process.env.ARCODEX_SERVER_PASSWORD = auth.password
  process.env.ARCODEX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.ARCODEX_SERVER_PASSWORD = undefined
  Flag.ARCODEX_SERVER_USERNAME = auth.username
  delete process.env.ARCODEX_SERVER_PASSWORD
  process.env.ARCODEX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
