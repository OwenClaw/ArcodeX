...
const managedConfigDir = process.env.ARCODEX_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "arcodex.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "arcodex.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "arcodex.jsonc"))
...
test("does not create global config when ARCODEX_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.ARCODEX_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.ARCODEX_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "arcodex.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.ARCODEX_CONFIG_DIR
    else process.env.ARCODEX_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "arcodex.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "arcodex.jsonc")
  ...
  try {
    ...
    await withTestInstance({
      ...
      fn: async (ctx) => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "arcodex.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ARCODEX_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ARCODEX_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ARCODEX_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ARCODEX_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "arcodex.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("does not try to install dependencies in read-only ARCODEX_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ARCODEX_CONFIG_DIR
  process.env.ARCODEX_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ARCODEX_CONFIG_DIR
    else process.env.ARCODEX_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ARCODEX_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(opencodeDir, "arcodex.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets ARCODEX_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
    ...
    if (urlStr.includes(".well-known/opencode")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/arcodex.json",
              headers: {
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(remoteFetchedUrl).toBe("https://config.example.com/arcodex.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "arcodex.json")
...
describe("ARCODEX_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ARCODEX_DISABLE_PROJECT_CONFIG"]
    process.env["ARCODEX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ARCODEX_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ARCODEX_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ARCODEX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ARCODEX_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ARCODEX_CONFIG_DIR"]
      process.env["ARCODEX_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ARCODEX_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ARCODEX_CONFIG_DIR"]
      } else {
        process.env["ARCODEX_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ARCODEX_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ARCODEX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ARCODEX_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ARCODEX_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ARCODEX_CONFIG_DIR"] = configDirTmp.path

      await withTestInstance({
        directory: projectTmp.path,
        fn: async (ctx) => {
          const config = await load(ctx)
          // Should load from ARCODEX_CONFIG_DIR, not project
...
describe("ARCODEX_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ARCODEX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ARCODEX_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ARCODEX_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ARCODEX_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ARCODEX_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ARCODEX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ARCODEX_CONFIG_CONTENT"]
