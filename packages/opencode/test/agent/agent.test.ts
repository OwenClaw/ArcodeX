...
it.instance("plan agent denies edits except .arcodex/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".arcodex/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
`,
      ...
      const home = process.env.ARCODEX_TEST_HOME
      process.env.ARCODEX_TEST_HOME = test.directory
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          process.env.ARCODEX_TEST_HOME = home
        }),
      )
