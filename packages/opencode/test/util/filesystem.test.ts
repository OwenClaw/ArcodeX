...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "arcodex.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "arcodex.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "arcodex.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "arcodex.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["arcodex.json", "arcodex.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "arcodex.json"),
        path.join(tmp.path, "arcodex.jsonc"),
        path.join(project, "arcodex.json"),
        path.join(project, "arcodex.jsonc"),
      ])
    })
  })
