...
const originalEnv = {
  ARCODEX_AUTH_CONTENT: process.env.ARCODEX_AUTH_CONTENT,
  ARCODEX_EXPERIMENTAL_WORKSPACES: process.env.ARCODEX_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  Database.close()
  restoreEnv()
  process.env.ARCODEX_EXPERIMENTAL_WORKSPACES = "true"
})
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (instance) => {
      process.env.ARCODEX_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      ...
      expect(JSON.parse(recorded.calls.create[0].env.ARCODEX_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.ARCODEX_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.ARCODEX_EXPERIMENTAL_WORKSPACES).toBe("true")
