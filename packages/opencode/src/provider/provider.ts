...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from arcodex.json provider config
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `arcodex auth cloudflare-ai-gateway`.",
        )
      }
    ...
    arcodex: Effect.fnUntraced(function* () {
      const auth = yield* dep.auth("arcodex")
      if (!auth) return { autoload: false }

      if (auth.type === "oauth") {
        if (auth.expires && Date.now() >= auth.expires) {
          log.info("arcodex token expired, attempting refresh")
          const { arcodexAuth, ACCESS_TOKEN_EXPIRES_MS } = yield* Effect.promise(() => import("@/plugin/arcodex"))
          const newTokens = yield* Effect.promise(() => arcodexAuth.refreshToken())
          if (newTokens) {
            log.info("arcodex token refresh successful")
            yield* Effect.promise(() =>
              import("@/plugin/arcodex").then((m) =>
                m.saveAuthToDisk("arcodex", {
                  type: "oauth",
          ...
          log.warn("arcodex token refresh failed, user needs to re-login")
          return { autoload: false }
        }
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // === ArcodeX: inject provider config if authenticated ===
        const arcodexAuth = yield* auth.get("arcodex").pipe(Effect.orElseSucceed(() => undefined))
        if (arcodexAuth && arcodexAuth.type === "oauth") {
          const { getArcodexProviderConfig } = yield* Effect.promise(() => import("@/plugin/arcodex-models"))
          const config = yield* Effect.promise(() => getArcodexProviderConfig(arcodexAuth.access))
          configProviders.push(["arcodex", config])
        }
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      // For arcodex provider, use small_model from dynamically fetched task_default_model_map (already cached)
      if (providerID === "arcodex") {
        const { getTaskDefaultModelMap } = yield* Effect.promise(() => import("@/plugin/arcodex-models"))
        const smallModel = getTaskDefaultModelMap().small_model
        return yield* getModel(ProviderID.make("arcodex"), ModelID.make(smallModel)).pipe(
          Effect.catchTag("ProviderModelNotFoundError", () => Effect.succeed(undefined)),
        )
      }
