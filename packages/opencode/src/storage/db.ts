...
declare const ARCODEX_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "arcodex.db")
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  return path.join(Global.Path.data, `arcodex-${safe}.db`)
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.ARCODEX_DB) {
    if (Flag.ARCODEX_DB === ":memory:" || path.isAbsolute(Flag.ARCODEX_DB)) return Flag.ARCODEX_DB
    return path.join(Global.Path.data, Flag.ARCODEX_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof ARCODEX_MIGRATIONS !== "undefined"
        ? ARCODEX_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof ARCODEX_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
