...
import { EmptyBorder, SplitBorder, paintGradientBorder } from "@tui/component/border"
...
import { assign, expandPastedTextPlaceholders, reconstructPromptFromParts } from "./part"
...
export function Prompt(props: PromptProps) {
  ...
  let borderBox: BoxRenderable | undefined
  // 每次进入时为渐变起点随机挑一种装饰色；渐变终点（主体色）始终跟随当前 Agent
  const gradientFrom = [
    RGBA.fromHex("#22d3ee"), // cyan
    RGBA.fromHex("#06b6d4"), // cyan-600
    RGBA.fromHex("#2dd4bf"), // teal
    RGBA.fromHex("#14b8a6"), // teal-600
    RGBA.fromHex("#34d399"), // emerald
    RGBA.fromHex("#10b981"), // emerald-600
    RGBA.fromHex("#84cc16"), // lime
    RGBA.fromHex("#a3e635"), // lime-400
    RGBA.fromHex("#eab308"), // yellow
    RGBA.fromHex("#fde047"), // yellow-300
    RGBA.fromHex("#3b82f6"), // blue
    RGBA.fromHex("#60a5fa"), // blue-400
    RGBA.fromHex("#6366f1"), // indigo
    RGBA.fromHex("#818cf8"), // indigo-400
    RGBA.fromHex("#a855f7"), // purple
    RGBA.fromHex("#c084fc"), // purple-400
    RGBA.fromHex("#ec4899"), // pink
    RGBA.fromHex("#f472b6"), // pink-400
  ...
  const promptCommands = createMemo(() =>
    [
        ...
        run: async () => {
          ...
          if (store.interrupt >= 2) {
            ...
            if (message) {
              ...
              ref.set(reconstructPromptFromParts(parts ?? []))
            }
            setStore("interrupt", 0)
          }
          dialog.clear()
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.ARCODEX_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false}>
        <box
          ref={(r: BoxRenderable) => (borderBox = r)}
          renderAfter={(buffer) => {
            if (borderBox) paintGradientBorder(buffer, borderBox, gradientFrom, highlight())
          }}
