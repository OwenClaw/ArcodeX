...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from arcodex.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "arcodex"),
    ...(await Filesystem.findUp(["arcodex.json", "arcodex.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "arcodex"))
  }
  if (Flag.ARCODEX_CONFIG) files.push(Flag.ARCODEX_CONFIG)
