...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old arcodex.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.arcodex`
  // folders between cwd and home, and ARCODEX_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ARCODEX_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ARCODEX_TUI_CONFIG override, if set.
  if (Flag.ARCODEX_TUI_CONFIG) {
    const configFile = Flag.ARCODEX_TUI_CONFIG
  ...
  // 4. `.arcodex` directories (and ARCODEX_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".arcodex") || dir === Flag.ARCODEX_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".arcodex") && dir !== Flag.ARCODEX_CONFIG_DIR) continue
