...
const TIPS: Tip[] = [
  ...
  "Create {highlight}arcodex.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/arcodex/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.arcodex/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.arcodex/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.arcodex/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.arcodex/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent ArcodeX from reading sensitive files",
  "Use {highlight}arcodex run{/highlight} for non-interactive scripting",
  "Use {highlight}arcodex --continue{/highlight} to resume the last session",
  "Use {highlight}arcodex run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}arcodex serve{/highlight} for headless API access to ArcodeX",
  "Use {highlight}arcodex run --attach{/highlight} to connect to a running server",
  "Run {highlight}arcodex upgrade{/highlight} to update to the latest version",
  "Run {highlight}arcodex auth list{/highlight} to see all configured providers",
  "Run {highlight}arcodex agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.arcodex/themes/{/highlight} directory",
  ...
  "Run {highlight}arcodex debug config{/highlight} to troubleshoot configuration",
