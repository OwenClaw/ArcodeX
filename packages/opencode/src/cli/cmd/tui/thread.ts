...
declare global {
  const ARCODEX_WORKER_PATH: string
}
...
async function target() {
  if (typeof ARCODEX_WORKER_PATH !== "undefined") return ARCODEX_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start arcodex tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start arcodex in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [ARCODEX_PROCESS_ROLE]: "worker",
        [ARCODEX_RUN_ID]: ensureRunID(),
      })
