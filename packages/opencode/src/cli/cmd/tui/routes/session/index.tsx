...
import { reconstructPromptFromParts } from "../../component/prompt/part"
...
import { formatCliHelpBannerLogoBlock } from '../../component/banner-logo';
...
export function Session() {
  ...
  createEffect(() => {
    ...
    const logoBlock = formatCliHelpBannerLogoBlock(dimensions().width);
    ...
    const tokenLine =
      tokens && total > 0
        ? `  ${weak('Tokens')}${UI.Style.TEXT_NORMAL_BOLD}${Locale.number(total)}${UI.Style.TEXT_NORMAL}  ${UI.Style.TEXT_DIM}(in ${Locale.number(tokens.input)} · out ${Locale.number(tokens.output)} · cache ${Locale.number(tokens.cache.read + tokens.cache.write)})${UI.Style.TEXT_NORMAL}`
        : undefined
    const lines = [
      '',
      logoBlock,
      ...
      `  ${weak('Continue')}${UI.Style.TEXT_NORMAL_BOLD}arcodex -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
    ]
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const parts = sync.data.part[message.id]
        prompt?.set(reconstructPromptFromParts(parts ?? []))
        dialog.clear()
      },
    },
    {
