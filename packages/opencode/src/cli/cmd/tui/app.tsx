...
function rendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.ARCODEX_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
    ...
    void (async () => {
      const choice = await DialogConfirm.show(
        ...
        [
          ...
          "",
          "Select Reset to review and clear local auth storage now. You can also exit and run `arcodex auth reset` manually.",
          "",
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.ARCODEX_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.ARCODEX_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("ArcodeX")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`ArcodeX | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`ArcodeX | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
...

      {
        ...
        title: "Open docs",
        run: () => {
          open("https://github.com/anthropic-ai/arcodex").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
      ...
      {
        name: "vim.toggle",
        title: kv.get("vim_enabled", false) ? "Disable Vim mode" : "Enable Vim mode",
        category: "System",
        run: async () => {
          const next = !kv.get("vim_enabled", false)
          kv.set("vim_enabled", next)
          if (next) {
            await TuiPluginRuntime.activatePlugin("internal:vim")
          } else {
            await TuiPluginRuntime.deactivatePlugin("internal:vim")
          }
          dialog.clear()
        },
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to ArcodeX v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.ARCODEX_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.ARCODEX_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
