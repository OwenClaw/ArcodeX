...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in arcodex.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .arcodex/ subdirectory too)
  const candidates = [path.join(baseDir, "arcodex.json"), path.join(baseDir, "arcodex.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".arcodex", "arcodex.json"), path.join(baseDir, ".arcodex", "arcodex.jsonc"))
  }
  ...
  // Default to arcodex.json if none exist
  return candidates[0]
}
