...
import { FOX_WIDTH } from "@/cli/fox-logo"
import type { RunSplashTheme } from "./theme"

const hasAnsi = (line: string) => line.includes("\x1b[")
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    for (let i = 0; i < logo.left.length; i += 1) {
      ...
      const rightLeft = hasAnsi(leftText) ? FOX_WIDTH + 1 : leftText.length + 1

      if (hasAnsi(rightText)) {
        push(lines, rightLeft, i, rightText, right)
      } else {
  ...
  if (kind === "exit") {
    ...
    const top = 1
    const body_left = (stripAnsi(mark[0] ?? "").length) + 2
    const session = "Session  "
    const label = "Continue "

    for (let i = 0; i < mark.length; i += 1) {
      const markLine = mark[i] ?? ""
      if (hasAnsi(markLine)) {
        push(lines, 0, top + i, markLine, left)
      } else {
        draw(lines, markLine, {
          left: 0,
          top: top + i,
