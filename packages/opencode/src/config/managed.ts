...
const MANAGED_PLIST_DOMAIN = "ai.arcodex.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/arcodex"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "arcodex")
    default:
      return "/etc/arcodex"
  }
}
...
export function managedConfigDir() {
  return process.env.ARCODEX_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
