  ...
  agreement: Schema.optional(
    Schema.Struct({
      ...
      privacy_url: Schema.optional(Schema.String).annotate({
        description: "ArcodeX AI privacy statement URL. Overrides built-in default.",
      }),
      terms_url: Schema.optional(Schema.String).annotate({
        description: "ArcodeX AI user agreement URL. Overrides built-in default.",
      }),
    ...
    description:
      "Agreement/legal configuration for ArcodeX onboarding. All fields optional — absent values fall back to built-in defaults.",
  }),
...
function globalConfigFile() {
  const candidates = ["arcodex.jsonc", "arcodex.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      if (!Flag.ARCODEX_CONFIG && !Flag.ARCODEX_CONFIG_DIR && !Flag.ARCODEX_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "arcodex.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "arcodex.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ARCODEX_CONFIG_CONTENT") return "local"
        ...
        if (Flag.ARCODEX_CONFIG) {
          yield* merge(Flag.ARCODEX_CONFIG, yield* loadFile(Flag.ARCODEX_CONFIG))
          log.debug("loaded custom config", { path: Flag.ARCODEX_CONFIG })
        }

        if (!Flag.ARCODEX_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("arcodex", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ARCODEX_CONFIG_DIR) {
          log.debug("loading config from ARCODEX_CONFIG_DIR", { path: Flag.ARCODEX_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".arcodex") || dir === Flag.ARCODEX_CONFIG_DIR) {
            for (const file of ["arcodex.json", "arcodex.jsonc"]) {
          ...
          // Auto-discovered plugins under `.arcodex/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ARCODEX_CONFIG_CONTENT) {
          const source = "ARCODEX_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ARCODEX_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from ARCODEX_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ARCODEX_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ARCODEX_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ARCODEX_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ARCODEX_PERMISSION))
        }
        ...
        if (Flag.ARCODEX_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ARCODEX_DISABLE_PRUNE) {
