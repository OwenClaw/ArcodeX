...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.ARCODEX_PID = String(process.pid)

    Log.Default.info("arcodex", {
    ...
    // Use the real database path (which varies by install channel / ARCODEX_DB)
    // so the one-time migration gate matches the DB the app actually opens.
    // Hardcoding "arcodex.db" mismatches channel-scoped DBs (e.g. arcodex-local.db)
    // and re-runs the migration — and its message — on every launch.
