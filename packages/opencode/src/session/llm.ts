...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      const opencodeProjectID = input.model.providerID.startsWith("opencode") || input.model.providerID.startsWith("arcodex")
        ? (yield* InstanceState.context).project.id
        : undefined

      const requestHeaders = {
        ...(input.model.providerID.startsWith("opencode") || input.model.providerID.startsWith("arcodex")
          ? {
    ...(opencodeProjectID ? { "x-arcodex-project": opencodeProjectID } : {}),
    "x-arcodex-session": input.sessionID,
    "x-arcodex-request": input.user.id,
    "x-arcodex-client": flags.client,
              "User-Agent": `arcodex/${InstallationVersion}`,
            }
          : {
